...
**IgnisFlux** 是一款 C++ 后台守护进程。  
它能根据实时的电池状态和用户自定义的阈值，动态管理系统的**温控策略**、**充电电流**和**过热保护**。
...
├── params/                                 # 所有用户自定义配置及目录
   ...
   ├── overheat_protect                     # 过热保护开关
   ├── trigger_overheat_threshold           # 触发过热限流的温度（℃）
   ├── dismiss_overheat_threshold           # 解除过热限流的温度（℃）
   ├── overheat_charge_current              # 过热时的充电电流（μA）
├── IgnisFlux                               # C++ 二进制文件
├── module.prop
└── service.sh
```
...
> - 步长：必须为`100000`的整数倍 (例如`3000000` = `3000mA`).

### 3. 过热保护

| 文件                          | 示例值     | 说明                                    |
|-------------------------------|------------|------------------------------------------|
| overheat_protect              | 1/0 或 true/false | 是否启用过热保护                      |
| trigger_overheat_threshold    | 43         | 触发过热限流的温度（℃）                  |
| dismiss_overheat_threshold    | 40         | 解除过热限流的温度（℃）                  |
| overheat_charge_current       | 1000000    | 过热时的充电电流限制（单位：μA）         |

> **工作原理**：
> - 过热保护与普通电流控制（`is_control_current`）**互相独立**，且**优先级更高**。
> - 当电池温度达到 `trigger_overheat_threshold` 时，充电电流被限制为 `overheat_charge_current`，不受普通电流控制影响。
> - 当电池温度低于 `dismiss_overheat_threshold` 时，解除限制，控制权交还给普通电流控制。
> - 仅在设备充电时生效。
> - 如果同时关闭 `overheat_protect` 和 `is_control_current`，充电电流恢复为系统默认（22A）。
> - 过热电流值支持设为 `0`（完全停止充电），且不要求步长为 `100000` 的整数倍。
> - 范围：`0` ~ `22000000` (`0mA` ~ `22000mA`).
